...
public class LogoLayer extends CCLayer {
    ...
    private void showGuhanLogo() {
        CGSize winSize = CCDirector.sharedDirector().getWinSize();

        // 创建黑色背景，完全覆盖屏幕
        CCSprite blackBg = CCSprite.sprite("cg/bg.jpg");
        blackBg.setPosition(winSize.getWidth()/2, winSize.getHeight()/2);
        blackBg.setScaleX(winSize.getWidth()/blackBg.getTextureRect().size.getWidth());
        blackBg.setScaleY(winSize.getHeight()/blackBg.getTextureRect().size.getHeight());
        addChild(blackBg);

        // 显示"孤寒"艺术字，完全覆盖屏幕
        CCSprite guhanText = CCSprite.sprite("logo/guhan.png");
        guhanText.setPosition(winSize.getWidth()/2, winSize.getHeight()/2);
        // 拉伸图片完全覆盖屏幕
        guhanText.setScaleX(winSize.getWidth() / guhanText.getTextureRect().size.getWidth());
        guhanText.setScaleY(winSize.getHeight() / guhanText.getTextureRect().size.getHeight());
        addChild(guhanText);

        CCDelayTime ccDelayTime = CCDelayTime.action(2.5f);
        ...
        guhanText.runAction(ccSequence);
    }
