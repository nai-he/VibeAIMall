...
public class VData {
	...
	private boolean ensureMoveState() {
		if (MDraw.logicMap == null || MDraw.logicMap.length == 0
				|| MDraw.logicMap[0] == null || MDraw.logicMap[0].length == 0
				|| so == null || pe == null) {
			return false;
		}
		normalizeMoveState();
	...
	private void normalizeMoveState() {
		clampPoint(so);
		clampPoint(pe);
		xIndex = clamp(xIndex, 0, Math.max(0, mapMaxX() - dosX + 1));
		yIndex = clamp(yIndex, 0, Math.max(0, mapMaxY() - dosY + 1));
	}

	private void clampPoint(APoint point) {
		if (point == null || MDraw.logicMap == null || MDraw.logicMap.length == 0) {
			return;
		}
		point.y = clamp(point.y, 0, mapMaxY());
		point.x = clamp(point.x, 0, mapMaxX());
	}

	private boolean isInLogicMap(int x, int y) {
		return MDraw.logicMap != null && y >= 0 && y < MDraw.logicMap.length
				&& MDraw.logicMap[y] != null && x >= 0 && x < MDraw.logicMap[y].length;
	}
	...
	private int mapMaxY() {
		...
		return MDraw.logicMap.length - 1;
	}
	...
	private int mapMaxX() {
		...
		return MDraw.logicMap[0].length - 1;
	}
