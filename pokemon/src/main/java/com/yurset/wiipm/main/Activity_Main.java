...
public class Activity_Main extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_wiipm_main);
	...
	private void handleView(int v_index) {
		if (v_index == FPublic.DEX_MAIN) {
			...
			int waitCount = 0;
			while (IGame._info == null && waitCount < 50) {
				try {
					Thread.sleep(100);
					waitCount++;
			...
			if (IGame._info == null) {
				android.util.Log.e("Activity_Main", "Failed to initialize IGame._info after 5 seconds");
				return;
			}
			_game.playerF.pause();
			_game.playerN.start();
