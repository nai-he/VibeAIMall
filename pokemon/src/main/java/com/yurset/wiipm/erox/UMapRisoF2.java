...
public class UMapRisoF2 extends RMapBase {
	...
	public UMapRisoF2(Context context, boolean iniFlag) {
		super(context);
		VData.relySize(11, 19);
		logicMap = FNodeBase.risof2;
		...
		spriteOdd10 = new Bitmap[135];
		sprite10 = FPublic.CreateBitmap(context, R.drawable.roomf20,
				15 * VData.UNIT, 9 * VData.UNIT);
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 15; j++) {
				spriteOdd10[15 * i + j] = Bitmap.createBitmap(sprite10,
			...
			for (int yy = 0; yy < VData.allSIZE.y; yy++) {
				for (int xx = 0; xx < VData.allSIZE.x; xx++) {
					switch (logicMap[yy][xx]) {
						case VData.TAG_FIGHT:// 4
						case VData.TAG_MASTER:// 6
						case VData.TAG_AFFAIR:// 8
						case 10:
						case 100:
						case 600:
