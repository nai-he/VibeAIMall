...
public class RMapCityF extends RMapBase {
	...
	private int pointer12 = 0;
	private Bitmap sprite12 = null;
	private Bitmap[] spriteOdd12 = null;
	...
	public RMapCityF(Context context, boolean iniFlag) {
		super(context);
		VData.relySize(32, 32);
		logicMap = FMakeBaseCity.city0f;
		...
		spriteOdd12 = new Bitmap[42];
		sprite12 = FPublic.CreateBitmap(context, R.drawable.spritex002,
				7 * VData.UNIT, 6 * VData.UNIT);
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 7; j++) {
				spriteOdd12[7 * i + j] = Bitmap.createBitmap(sprite12,
						VData.UNIT * j, VData.UNIT * i, VData.UNIT, VData.UNIT);
			}
		}
			...
			for (int yy = 0; yy < VData.allSIZE.y; yy++) {
				for (int xx = 0; xx < VData.allSIZE.x; xx++) {
					switch (logicMap[yy][xx]) {
						...
						case 12:
						case 902:
							imageMap1[yy + 1][xx + 1] = spriteOdd12[pointer12];
							pointer12++;
							if (pointer12 == 42) {
								pointer12 = 0;
							}
				...
				for (int xx = 0; xx < VData.allSIZE.x; xx++) {
					switch (logicMap[yy][xx]) {
						...
						case 211:
							...
							break;
						case 212:
							imageMap2[yy + 1][xx + 1] = spriteOdd12[pointer12];
							pointer12++;
							if (pointer12 == 7) {
								pointer12 = 0;
							}
